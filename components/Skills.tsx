...
const Skills = () => {
  ...
  const skillCategories = [
    ...
    },
    {
      ...
      skills: [
        ...
        { name: 'Move Programming', level: 70 },
      ],
    },
  ];
  ...
  const radarData = [
    ...
    { skill: 'TypeScript', value: 85, fullMark: 100 },
    { skill: 'JavaScript', value: 90, fullMark: 100 },
    { skill: 'PHP', value: 85, fullMark: 100 },
    { skill: 'Laravel', value: 88, fullMark: 100 },
    { skill: 'MySQL', value: 88, fullMark: 100 },
    { skill: 'CSS3', value: 90, fullMark: 100 },
    { skill: 'GitHub', value: 90, fullMark: 100 },
  ];

  const overallProficiency = 88;
                ...
                <div className="relative bg-gradient-to-br from-dark-800/90 to-dark-900/90 backdrop-blur-xl rounded-2xl p-6 border border-dark-700/50 group-hover:border-primary-500/50 transition-all duration-300 shadow-xl group-hover:shadow-2xl h-full">
                  ...
                  <div className="space-y-4">
                    {category.skills.map((skill, skillIndex) => (
                      <motion.div key={skill.name} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 + skillIndex * 0.04 }}>
                        <div className="flex justify-between items-center mb-2">
                          <span className="text-gray-300 font-medium text-sm hover:text-white transition-colors">{skill.name}</span>
                          <span className="text-primary-400 font-semibold text-xs">{skill.level}%</span>
                        </div>
                        <div className="h-2 bg-dark-700/50 rounded-full overflow-hidden">
                          <motion.div
                            initial={{ width: 0 }}
                            animate={inView ? { width: `${skill.level}%` } : { width: 0 }}
                            transition={{ duration: 1, delay: categoryIndex * 0.08 + skillIndex * 0.04, ease: 'easeOut' }}
